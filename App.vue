<script>
// 模块级函数，避免依赖 this.methods（App.vue 在 uni-app Vue3 中不一定支持）
async function fetchRole(globalData) {
  try {
    ...
    if (r.success) {
      ...
      console.log('[role]', r.role, '[openid]', r.openid)
    } else {
      console.warn('[getRole] result not success:', r)
    }
  } catch (err) {
...
page {
  ...
  background: linear-gradient(180deg, #EAF4FE 0%, #F1F7FD 30%, #F8FAFC 100%);
...
.btn-primary {
  ...
  width: 100%;
  margin-top: 32rpx;
  box-shadow: 0 10rpx 24rpx rgba(28, 176, 246, 0.35),
              0 4rpx 0 #0E8CBF;
  letter-spacing: 2rpx;
  position: relative;
  top: 0;
...
.btn-primary:active {
  top: 4rpx;
  box-shadow: 0 4rpx 12rpx rgba(28, 176, 246, 0.25),
              0 2rpx 0 #0E8CBF;
}
...
.input-field {
  ...
  color: var(--text-primary);
  overflow: visible;
  box-shadow: inset 0 2rpx 4rpx rgba(15, 23, 42, 0.02);
  transition: all 0.18s ease;
}

.input-field:focus {
  border-color: var(--primary);
  box-shadow: 0 0 0 6rpx rgba(28, 176, 246, 0.15);
}
