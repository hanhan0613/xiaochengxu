/**
 * QR Code Generator (标准版，含完整 Reed-Solomon 纠错码)
 * 基于 kazuhikoarase/qrcode-generator 的算法（MIT License）适配 Canvas 2D
 *
 * 用法:
 *   import QRCode from '@/utils/qrcode.js'
 *   QRCode.draw(text, canvasNode, ctx, size)
 */
...
;(function () {
  for (let i = 0; i < 8; i++) QRMath_EXP_TABLE[i] = 1 << i
  for (let i = 8; i < 256; i++) {
    QRMath_EXP_TABLE[i] =
      QRMath_EXP_TABLE[i - 4] ^
      QRMath_EXP_TABLE[i - 5] ^
      QRMath_EXP_TABLE[i - 6] ^
      QRMath_EXP_TABLE[i - 8]
  }
  for (let i = 0; i < 255; i++) QRMath_LOG_TABLE[QRMath_EXP_TABLE[i]] = i
})()
const QRMath = {
  glog(n) {
...
// ---------- Error Correct Blocks (RS_BLOCK_TABLE) ----------
// 每个 version 对应 4 种纠错等级的 (blocks, totalPerBlock, dataPerBlock)
...
QRCodeModel.getLengthInBits = function (mode, type) {
  // 8bit byte mode length indicator bits
...
// ---------- 根据数据长度自动选择合适 version ----------
// L 级纠错 8-bit byte 模式下每个 version 可容纳字节数
...
function autoVersion(byteLen) {
  for (let i = 0; i < BYTE_CAPACITY_L.length; i++) {
    if (byteLen <= BYTE_CAPACITY_L[i]) return i + 1
  }
